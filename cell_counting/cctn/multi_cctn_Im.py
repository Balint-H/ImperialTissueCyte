...
def count_process(slice_number_cp, count_file, structure_cp, circ_thresh=0.8, bg_thresh = 6., size = 85., radius = 12.):
    ...
    with Image.open(count_path + '/' + count_file) as image:
        ...
        if mask:
            ...
            mask_image = np.array(Image.fromarray(seg[slice_number_cp]).resize((temp_shape[1],temp_shape[0]), Image.NEAREST))
            ...
            im_out = np.multiply(np.divide(image, np.max(image)), 255.)
            im_out=im_out.astype('uint8')
            
            im_out = Image.fromarray(im_out, mode='L')
            orig=im_out;
            #im_out.save(count_path+"/counts_v5_Balint/masked_"+str(slice_number_cp)+".tif")
        ...
        if use_medfilt:
            ...
            im_out = Image.fromarray(im_out, mode='L')
            #im_out.save(count_path+"/counts_v5_Balint/rolled_"+str(slice_number_cp)+".tif")
        ...
        if image.shape[0] * image.shape[1] > (radius * 2) ** 2 and np.max(image) != 0.:
            ...
            im_out = Image.fromarray(im_out, mode='L')
            im_out.save(count_path+"/counts_v5_Balint/balled_"+str(slice_number_cp)+".tif")
            if np.max(image) != 0.:
                ...
                if new_circ:
                    ...
                    im_out = Image.fromarray(im_out, mode='L')
                   # im_out.save(count_path+"/counts_v5_Balint/thresh_"+str(slice_number_cp)+".tif")
                else:
                ...
                if mask:
                    ...
                    for ii, _ in enumerate(areas):
                        if areas[ii] > size and areas[ii] < size * 10 and circ[ii] > 0.65:
                            ...
                            cur_cell=centroids[ii]
                            draw.rectangle([(cur_cell[1] - 20, cur_cell[0]-1), (cur_cell[1] + 20, cur_cell[0]+1)], fill="red")
                            draw.rectangle([(cur_cell[1], cur_cell[0]-1 - 20), (cur_cell[1]+1, cur_cell[0] + 20)], fill="red")
        else:
            cells = []
        orig.save(count_path+"/counts_v5_Balint/orig_"+str(slice_number_cp)+".png")
